...
const createEmptySlot = (id: number, name?: string): Slot => ({
  id,
  name: name || `저장소 ${Math.floor(Math.random() * 1000)}`,
  topics: [],
  lastDrawnId: null
});

const INITIAL_DATA: AppData = {
  1: { id: 1, activeSlotId: 0, slots: [createEmptySlot(Date.now(), '저장소 1')] },
  2: { id: 2, activeSlotId: 0, slots: [createEmptySlot(Date.now() + 1, '저장소 1')] },
  3: { id: 3, activeSlotId: 0, slots: [createEmptySlot(Date.now() + 2, '저장소 1')] }
};
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('debate_draw_data_v4');
      if (saved) {
  ...
  useEffect(() => {
    if (isLoaded) {
      try {
        localStorage.setItem('debate_draw_data_v4', JSON.stringify(data));
      } catch (e) {
        ...
        <div className="max-w-5xl mx-auto">
          <header className="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-6">
            <div className="text-center sm:text-left">
              <span className="text-[10px] font-bold text-indigo-500 uppercase tracking-widest bg-indigo-50 px-3 py-1 rounded-full mb-2 inline-block">
                {dayLabels[activeDay]}
              </span>
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl shadow-slate-200/60 p-5 md:p-10 min-h-[500px] md:min-h-[600px] transition-all duration-300 border border-slate-100 relative overflow-hidden">
